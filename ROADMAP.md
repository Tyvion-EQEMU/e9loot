...
- **Diagnostics / Debug Output** — The Console panel is built. Remaining ideas: a session stats summary (items kept/sold/destroyed, plat value); exporting loot history to CSV; a `/e9loot debug` slash command to dump current config, list sizes, and script state in one shot.
...
- **SellStuff / Restock** — Two remaining LootNScoot features:
  ...
  - ~~*Bank* — Walk to a banker and deposit items tagged as `bank`. **Implemented as `/e9loot bankstuff`** — see Completed above.~~
  - All three would benefit from a `/e9loot sell`, `/e9loot restock` slash command as well as buttons in the panel.
